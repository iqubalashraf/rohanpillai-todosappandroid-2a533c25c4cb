...
public class ItemsDatabase extends SQLiteOpenHelper {
    ...
    // Adding new Note
    public boolean addNote(String note_title, String note_description, String image_path, String thumbnail_path ,int status, int category_id, int count) {
        try {
            ...
            values.put(KEY_THUMBNAIL_PATH, thumbnail_path);
    ...
    // Updating old Note
    public boolean updateNote(int id, String note_title, String note_description, String image_path, String thumbnail_path , int status) {
        try {
            SQLiteDatabase db = this.getWritableDatabase();
        ...
        try {
            ...
            if (cursor.moveToFirst()) {
                do {
                    ...
                    note.setThumbnail_uri(cursor.getString(4));
                    note.setStatus(cursor.getInt(5));
                    note.setCategory_id(cursor.getInt(6));
                    notes.add(note);
